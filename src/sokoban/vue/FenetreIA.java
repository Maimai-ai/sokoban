...
/**
	* FenetreIA est une classe qui hérite de JFrame et implements ActionListener ainsi que Observer
	* Elle observe deux Plateau
*/

public class FenetreIA extends JFrame implements ActionListener, Observer{
	...
	public FenetreIA(String unfichier){
		super();
		...
		ControleurToucheIA touche = new ControleurToucheIA(this.player1, this.player2);
		
		this.vuejoueur1.addKeyListener(touche);
		//bouge mais dans le terminal
	...
	public void actionPerformed(ActionEvent e){
		...
		if (appuiesur == exit){
			...
			new FenetreIA(this.unfichier).setVisible(true);
			this.dispose();
		}
	}
	...
	*/
	public void passageNivSuiv(String niveau){
		...
		int i = (int) tab[13];
		if(i==57){
			new FenetreIA("niveau/niveau10.xsb").setVisible(true);
			this.dispose();
		}
		else{
			if(tab.length!=18){
				...
				char n = (char) i;
				tab[13] = n;
				new FenetreIA(String.valueOf(tab)).setVisible(true);
				this.dispose();
			}
		}
	}
